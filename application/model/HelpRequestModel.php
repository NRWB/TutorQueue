...
class HelpRequestModel {
  ...
  public static function createHelpRequest($subj, $subsubj, $tutorReq) {
    ...
    if (strlen($subsubj) > 0 && strlen($tutorReq) > 0) {
      ...
      $query = $database->prepare($sql);
      $query->execute(array(':table_num' => '1', ':subj_DD' => $subj, ':sub_subj_DD' => $subsubj, ':req_tutor_DD' => $tutorReq));
      if ($query->rowCount() == 1) {
        return true;
      }
      ...
      $query = $database->prepare($sql);
      $query->execute(array(':table_num' => '1', ':subj_DD' => $subj, ':sub_subj_DD' => $subsubj));
      if ($query->rowCount() == 1) {
        return true;
      }
    } elseif (strlen($subsubj) == 0 && strlen($tutorReq) > 0) {
      ...
      $query = $database->prepare($sql);
      $query->execute(array(':table_num' => '1', ':subj_DD' => $subj, ':req_tutor_DD' => $tutorReq));
      if ($query->rowCount() == 1) {
        return true;
      }
    } else {
      ...
      $query = $database->prepare($sql);
      $query->execute(array(':table_num' => '1', ':subj_DD' => $subj));
      if ($query->rowCount() == 1) {
        return true;
      }
    }
